...
/**
 *	\var		_opAri[]
 *	\brief		Tableau des opérateurs arithmétiques.
 */
static struct
{
    char *opName;
...
void installDefaultSymbols(void)
{
    ...
    printMessage(5);
    for (i = 0; _opAri[i].opName != NULL; i++)
        installSymbol(_opAri[i].opName, PRG, _opAri[i].opType, sizeof(double), (generic)_opAri[i].opPtrFunc);
    printMessage(9);
}
